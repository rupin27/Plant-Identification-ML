...
def split_test_train(data, ranNum):
...
def cart(dataAttr, colAttr):
    ...
    for attr in colAttr[:-1]:
        ...
        for value in attrVals:
            ...
            giniVar += len(attrLabels) / len(dataAttr[-1]) * gini(attrLabels)
...
def constructDecisionTree(data, colAttr, algor):
    ...
    node = descisionTreeClass(label = -1, nodeType = "head")
...
def boolPred(tree: descisionTreeClass, instance):
    ...
    if instance[tClassIdx] not in tree.nodeEdges:
        return predVal == corrVal, predVal, corrVal
...
def accuracy(data, algorithm, ranNum):
    ...
    for i in range(1, 101):
        ...
        housetrain, housetest = split_test_train(houseVotesData, 12200 + ranNum * i)
        traintree = constructDecisionTree(housetrain, dataCols, algorithm)
...
plotTrainID3 = np.array(accuracy('train', 'id3', 200))
# print(plotTrainID3)
...
plotTestID3 = np.array(accuracy('test', 'id3', 200))
# print(plotTestID3)
...
plotTrainGini = np.array(accuracy('train', 'cart', 200))
# print(plotTrainGini)
...
# QE.1 Repeat the experiments Q2.1 to Q2.4, but now use the Gini criterion for node splitting, instead of the Information Gain criterion.
plotTestGini = np.array(accuracy('test', 'cart', 200))
# print(plotTestGini)
